...
using Lucia.Table.V2.share.attrib;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    internal class EquipModule
    {
        ...
        public static void EquipBreakthroughRequestHandler(Session session, Packet.Request packet)
        {
            ...
            var equipBreakTables = TableReaderV2.Parse<EquipBreakThroughTable>();

            var equipBreakThrough =
                equipBreakTables.Find(x => x.EquipId == equip.TemplateId && x.Times == equip.Breakthrough);

            if (equipBreakThrough != null &&
                equipBreakTables.Any(x => x.EquipId == equip.TemplateId && x.Times == equip.Breakthrough + 1))
            {
                NotifyItemDataList notifyItemData = new();

                var itemIds = equipBreakThrough.ItemId.ParseIntList();
                var itemCounts = equipBreakThrough.ItemCount.ParseIntList();

                for (int i = 0; i < Math.Min(itemIds.Count, itemCounts.Count); i++)
                    notifyItemData.ItemDataList.Add(session.inventory.Do(itemIds[i], itemCounts[i] * -1));

                if (equipBreakThrough.UseMoney != null && equipBreakThrough.UseMoney > 0)
            ...
            else if (equipBreakThrough != null)
            {
        ...
        public static void EquipResonanceRequestHandler(Session session, Packet.Request packet)
        {
            ...
            #region Pools
            var equipResonance = TableReaderV2.Parse<EquipResonanceTable>().Find(x => x.Id == equip.TemplateId);
            List<ResonanceInfo> resonancePool = new();

            /*var attribPoolDict = equipResonance?.AttribPoolId.ParseIntDict() ?? new();
            foreach (var kv in attribPoolDict)
            {
                var attribPool = TableReaderV2.Parse<AttribPoolTable>().Where(x => x.PoolId == kv.Key);
                foreach (var attrib in attribPool)
                    resonancePool.Add(new() { Slot = request.Slot, Type = EquipResonanceType.Attrib, TemplateId = attrib.Id });
            }*/

            var charSkillPoolDict = equipResonance?.CharacterSkillPoolId.ParseIntDict() ?? new();
            foreach (var kv in charSkillPoolDict)
            {
                var pool = TableReaderV2.Parse<CharacterSkillPoolTable>().Where(x => x.PoolId == kv.Key);
                foreach (var skill in pool)
                    resonancePool.Add(new() { Slot = request.Slot, Type = EquipResonanceType.CharacterSkill, TemplateId = skill.SkillId });
            }

            var weaponSkillPoolDict = equipResonance?.WeaponSkillPoolId.ParseIntDict() ?? new();
            var weaponPool = TableReaderV2.Parse<WeaponSkillPoolTable>();

            foreach (var kv in weaponSkillPoolDict)
            {
                foreach (var skill in weaponPool.Where(x => x.PoolId == kv.Key))
                {
                    foreach (var skillId in skill.SkillId.ParseIntList())
                    {
            ...
            if (request.UseItemId != null && request.UseItemId > 0)
            {
                var resonanceUseItem = TableReaderV2.Parse<EquipResonanceUseItemTable>().Find(x => x.Id == equip.TemplateId);
                if (resonanceUseItem == null)
                {
                ...
                NotifyItemDataList notifyItemData = new();
                var costIds = resonanceUseItem.ItemId.ParseIntList();
                var costCounts = resonanceUseItem.ItemCount.ParseIntList();

                for (int i = 0; i < Math.Min(costIds.Count, costCounts.Count); i++)
                    notifyItemData.ItemDataList.Add(session.inventory.Do(costIds[i], costCounts[i] * -1));

                session.SendPush(notifyItemData);
            }
            else if (request.UseEquipId != null && request.UseEquipId > 0)
            {
                throw new NotImplementedException();
            }

            var resonance = resonancePool[Random.Shared.Next(resonancePool.Count)];
