...
using Lucia.Table.V2.share.character.enhanceskill;
using Lucia.Table.V2.share.character.grade;
    ...
    internal class CharacterModule
    {
        ...
        public static void CharacterActivateStarRequestHandler(Session session, Packet.Request packet)
        {
            ...
            try
            {
                ...
                if (character.Star < characterQualityFragment.StarUseCount.ParseIntList().Count)
                {
        ...
        public static void CharacterUnlockSkillGroupRequestHandler(Session session, Packet.Request packet)
        {
            ...
            NotifyCharacterDataList notifyCharacterData = new();
            var affectedChars = TableReaderV2.Parse<CharacterSkillTable>().Where(x => x.SkillGroupId.ParseIntList().Contains(request.SkillGroupId)).Select(x => x.CharacterId);
        ...
        public static void CharacterUnlockEnhanceSkillRequestHandler(Session session, Packet.Request packet)
        {
            ...
            var enhanceSkillIds =
                TableReaderV2.Parse<EnhanceSkillGroupTable>()
                .Where(x => x.Id == request.SkillGroupId)
                .SelectMany(x => x.SkillId.ParseIntList())
                .ToList();

            var enhanceSkillTable = TableReaderV2.Parse<EnhanceSkillTable>();
            var enhanceUpgradeTable = TableReaderV2.Parse<EnhanceSkillUpgradeTable>();
            ...
            foreach (var enhanceSkillId in enhanceSkillIds)
            {
                var affectedChars =
                    enhanceSkillTable
                    .Where(x => x.SkillGroupId.ParseIntList().Contains((int)request.SkillGroupId))
                ...
                var upgradeTable =
                    enhanceUpgradeTable.Find(x => x.SkillId == enhanceSkillId && x.Level == 0);

                if (upgradeTable == null)
                    continue;

                var costItems = upgradeTable.CostItem.ParseIntList();
                var costCounts = upgradeTable.CostItemCount.ParseIntList();

                for (int i = 0; i < Math.Min(costItems.Count, costCounts.Count); i++)
                    notifyItemData.ItemDataList.Add(session.inventory.Do(costItems[i], costCounts[i] * -1));
            }
        ...
        public static void CharacterUpgradeEnhanceSkillRequestHandler(Session session, Packet.Request packet)
        {
            ...
            foreach (var enhanceSkillId in enhanceSkillIds)
            {
                foreach (var character in session.character.Characters.Where(x => x.EnhanceSkillList.Any(s => s.Id == enhanceSkillId)))
                {
                    for (int j = 0; j < request.Count; j++)
                    {
                        var skill = character.EnhanceSkillList.Find(s => s.Id == enhanceSkillId);
                        if (skill == null)
                            continue;

                        var upgradeTable =
                            enhanceUpgradeTable.Find(x => x.SkillId == enhanceSkillId && x.Level == skill.Level);
