...
using Lucia.Logging;
using Lucia.Table.V2.client.draw;
            ...
            switch (drawScene.Type)
            {
                case 1:
                    ...
                    if (random >= 0.972f)
                    {
                        ...
                        if (rate <= 0.3f)
                        {
                            ...
                            foreach (var charId in TableReaderV2.Parse<CharacterTable>().Select(x => x.Id))
                            {
                                ...
                                if (characterQuality.Quality >= 3 && drawPool.GoodsId.ParseIntList().Contains(character.Id))
                                {
                                    pool.Add(charId);
                                }
                            }
                            if (pool.Count < 1)
                            {
                                foreach (var charId in TableReaderV2.Parse<CharacterTable>().Select(x => x.Id))
                                {
                                    ...
                                    if (characterQuality.Quality >= 3 && !drawPool.UpGoodsId.ParseIntList().Contains(character.Id))
                                    {
                                        pool.Add(charId);
                                    }
                                }
                        ...
                        {
                            if (drawPool.UpGoodsId.ParseIntList().Count == 1)
                            {
                                CharacterTable? character = TableReaderV2.Parse<CharacterTable>()
                                             .Find(x => x.Id == drawPool.UpGoodsId.ParseIntList()[0]);

                                CharacterQualityTable? characterQuality = TableReaderV2.Parse<CharacterQualityTable>().OrderBy(x => x.Quality)
                                    .FirstOrDefault(x => x.CharacterId == drawPool.UpGoodsId.ParseIntList()[0]);
                                ...
                                rewards.Add(new()
                                {
                                    ...
                                    Quality = characterQuality?.Quality ?? 0,
                                    Count = 1,
                                    TemplateId = drawPool.UpGoodsId.ParseIntList()[0]
                                });
                            }
                            else if (drawPool.UpGoodsId.ParseIntList().Count > 0)
                            {
                                var upGoods = drawPool.UpGoodsId.ParseIntList();
                                int rand = upGoods[Random.Shared.Next(upGoods.Count)];
                    ...
                    else if (random >= 0.9f)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (var charId in TableReaderV2.Parse<CharacterTable>().Select(x => x.Id))
                            {
                                CharacterTable? character = TableReaderV2.Parse<CharacterTable>().Find(x => x.Id == charId);
                                CharacterQualityTable? characterQuality = TableReaderV2.Parse<CharacterQualityTable>().OrderBy(x => x.Quality)
                                    .FirstOrDefault(x => x.CharacterId == charId);
                                if (character is null || characterQuality is null)
                                    continue;

                                if (characterQuality.Quality == 2 && drawPool.GoodsId.ParseIntList().Contains(character.Id))
                                {
                                    pool.Add(charId);
                                }
                            }
                            if (pool.Count < 1)
                            {
                                foreach (var charId in TableReaderV2.Parse<CharacterTable>().Select(x => x.Id))
                                {
                                    ...
                                    if (characterQuality.Quality == 2 && !drawPool.UpGoodsId.ParseIntList().Contains(character.Id))
                                    {
                                        pool.Add(charId);
                                    }
                                }
                            ...
                            if (pool.Count > 0)
                            {
                                ...
                                CharacterTable? character = TableReaderV2.Parse<CharacterTable>().Find(x => x.Id == rand);
                                CharacterQualityTable? characterQuality = TableReaderV2.Parse<CharacterQualityTable>().OrderBy(x => x.Quality)
                                    .FirstOrDefault(x => x.CharacterId == rand);
                    ...
                    else if (random >= 0.82f)
                    {
                        ...
                        var qualities = TableReaderV2.Parse<CharacterQualityTable>();

                        foreach (var charId in characters.Select(x => x.Id))
                        {
                            CharacterTable? character = characters.Find(x => x.Id == charId);
                            CharacterQualityTable? characterQuality =
                                qualities.OrderBy(x => x.Quality)
                                         .FirstOrDefault(x => x.CharacterId == charId);

                            if (character is null || characterQuality is null)
                                continue;

                            if (characterQuality.Quality >= 3 &&
                                drawPool.GoodsId.ParseIntList().Contains(character.Id))
                            {
                                pool.Add(charId);
                            }
                        }
                        ...
                        if (pool.Count < 1)
                        {
                            foreach (var charId in TableReaderV2.Parse<CharacterTable>().Select(x => x.Id))
                            {
                                ...
                                if (characterQuality.Quality == 1 &&
                                    !drawPool.UpGoodsId.ParseIntList().Contains(character.Id))
                                {
                                    pool.Add(charId);
                                }
                            }
                    ...
                    else if (random >= 0.61f)
                    {
                        ...
                        pool.AddRange(drawPool.GoodsId.ParseIntList());
                        pool.AddRange(drawPool.UpGoodsId.ParseIntList());
                        ...
                        if (pool.Count > 0)
                        {
                            ...
                            var qualityByCharId = qualities
                                .GroupBy(x => x.CharacterId)
                                .ToDictionary(g => g.Key, g => g.OrderBy(q => q.Quality).First());

                            int rand = pool[Random.Shared.Next(pool.Count)];

                            var character = characters.Find(x => x.Id == rand);
                            qualityByCharId.TryGetValue(rand, out var characterQuality);
                            ...
                            if (character is not null)
                            {
                                rewards.Add(new()
                                {
                                    ...
                                    Count = characterQuality?.Quality >= 3 ? 3
                                         : characterQuality?.Quality == 2 ? 8
                                         : 12,
                                    Quality = characterQuality?.Quality ?? 0,
