...
namespace Lucia.Common
{
    public static class TableExtensions
    {
        public static int GetCharacterExp(this ItemTable item, int cardType)
        {
            var sub = item.SubTypeParams.ParseIntList();

            if (item.ItemType == (int)ItemType.CardExp && sub.Count >= 3)
            {
                int upType = sub[0];
                int exp = sub[1];
                int upPercentage = sub[2] / 100 - 100;
                int upMultiple = sub[2] / 10000;
        ...
        public static EquipUpgradeInfo GetEquipUpgradeInfo(this ItemTable item)
        {
            ...
            if (item.ItemType == (int)ItemType.EquipExp && sub.Count >= 3)
            {
                int classify = sub[0];
                int exp = sub[1];
                int cost = sub[2];
